...
ggplot(first_scaffols_positions, aes(position, after_stat(count), fill = niche, alpha=element)) +
  geom_density(position = "fill", adjust=1/5, color="white", size=0.2) + scale_fill_manual(values = met.brewer("Stevens")) + 
...
head data/sca_lengths.tab -n 16 | sed 's/Sc00000/S/' > sixt.sca.tab
...
```{r circos plot}
# GET CHOORDINATES
sff.bed = read.table("sixt.sca.tab")
...
circos.genomicInitialize(sff.bed, plotType = "labels", labels.cex = 8)
circos.track(ylim = c(0, 1), bg.border = NA, bg.col = met.brewer("Cross", 16) , track.height = 0.05)
...
upstream$te[upstream$te = "DNA"]
upstream = upstream %>% mutate(
    niche = case_when(
      te %in% c("DNA", "DNA/CMC-EnSpm", "DNA/CMC-Transib", "DNA/hAT-Ac", "DNA/hAT-Tag1", "DNA/hAT-Tip100", "DNA/IS3EU", "DNA/Kolobok-Hydra", "DNA/Maverick", "DNA/Merlin", "DNA/MuLE-MuDR", "DNA/MULE-MuDR", "DNA/TcMar-Tc2") ~ "Class_II",
      element == "exon" ~ "coding",
      T ~ "Class_I"
    )
  )
ggplot(upstream) + geom_boxplot(aes(x=len, y=te)) + 
  theme_hc()+
  labs(x="Lenght")+
...
```{r}
ggplot(upstream, aes(x=tis, y=te))+
   geom_density_ridges_gradient(alpha=0.4, quantile_lines = TRUE, quantiles=2) + 
  theme_hc()+
...
```{r}
ggplot(upstream) + geom_boxplot(aes(x=mean_exp, y=te))+
  theme_hc()+
  labs(x="TPM")+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
...
```{r}
ggplot(upstream) + geom_bar(aes(x=after_stat(count), y=te), fill="black") + 
  theme_hc()+
  labs(x="Count")+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
