...
ggplot(first_scaffols_positions, aes(position, after_stat(count), fill = niche, alpha=element)) +
  geom_density(position = "fill", adjust=1/5, color="white") + scale_fill_manual(values = met.brewer("Stevens")) + 
...
```
```{r}
ggplot(upstream, aes(x=tis, y=te, fill=..x..))+
...
```
```{r}
ggplot(upstream) + geom_bar(aes(x=after_stat(count), y=te))
```
