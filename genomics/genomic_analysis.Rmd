...
#Random sample of 30k TEs 

shuf tes.gff | head -n30000 > domains/sample_tes.gff > bed > fasta
TEsorter sample_tes.fasta -db rexdb-plant

Order           Superfamily      # of Sequences# of Clade Sequences    # of Clades# of full Domains
LTR             Copia                      6315           1735             11            281
LTR             Gypsy                      7533           2369             10            299
LTR             mixture                      91              0              0              0
pararetrovirus  unknown                      10              0              0              0
LINE            unknown                     253              0              0              0
TIR             EnSpm_CACTA                  30              0              0              0
TIR             MuDR_Mutator                208              0              0              0
TIR             hAT                         107              0              0              0
mixture         mixture                      23              0              0              0

#RT and TPase DOMAINS
cat sample_tes.fasta.rexdb-plant.dom.tsv | grep -P "\-RT\t" | get_record.py -i sample_tes.fasta.rexdb-plant.d
om.faa -o RT_domains.fasta -t fasta

cat sample_tes.fasta.rexdb-plant.dom.tsv | grep -P "\-TPase\t" | get_record.py -i sample_tes.fasta.rexdb-plan
t.dom.faa -o TPase_domains.fasta -t fasta

# RT TREE
mafft --auto RT_domains.fasta > RT_domains.aln
FFT-NS-2 (Fast but rough)

iqtree -s RT_domains.aln -bb 1000 -nt AUTO
Best-fit model: JTT+F+R5 chosen according to BIC

#TPase TREE
mafft --auto TPase_domains.fasta > TPase_domains.aln
L-INS-i (Probably most accurate, very slow)

iqtree -s TPase_domains.aln -bb 1000 -nt AUTO
...
ggsave(file="figures/landscape.png", width=12, height=6, dpi=600)
```

```{r stacked_bar}
#BARPLOT OF TOTAL COUNTS

orders <- read.table("data/orders.tab")
colnames(orders) = c("count", "order")
orders$order

orders = orders %>% mutate(
    class = case_when(
      order %in% c("LINE/Jockey", "LINE/L1",           
                   "LINE/L2",          
                   "LINE/Penelope",     
                   "LINE/R1",         
                   "LINE/RTE-BovB",     
                   ...
                   "LTR/Ngaro",         
                   "LTR/Pao",          
                   "SINE/Unknown" ) ~ "Class_I",
      T ~ "Class_II"
    )
  )

ggplot(orders) + geom_bar(aes(x= "", y= count, fill=class, alpha= order), position = "stack", stat = "identity", color="white") +
  scale_fill_manual(values = met.brewer("Stevens")) +
  labs(x="Whole genome", y= "TE order count")+
  guides(alpha="none", fill="none") +
  theme_few()+
  theme(text = element_text(size=30)) 
ggsave(file="figures/ordercount.png", width=5, height=30, dpi=300)
```

# CIRCOS PLOT
```{bash syntenic_blocks, eval=FALSE, include=FALSE}
# ALLVSALL BLAST
blastn -query mrnas.fasta -db database -evalue 1e-10 -num_alignments 5 -outfmt 6 -out agave.blast
# REARRENGE BED FILE TO Chromosome Gene_name Start End
awk -F '\t' '{print $1,$4,$2,$3}' OFS='\t' "mrnas.bed" > agave.gff
# RUN MCScanX
../MCScanX-master/MCScanX agave
# I DID IT ALSO FO THE 16 SFFS
# ISOLATION OF GENES IN THE LONGESTS SCAFFOLDS
          rm mcs_sca.gff
          for sc in $(cut -f 1 longer.sca.tab)
          do
          echo "Searching $sc"
          cut -f 1-9 gff/mrnas.gff | grep -w "$sc" >> 16.gff
          done
# AND THEN THE ABOVE PROCEDURES
54 alignments generated
# GET THE BLOCKS
tail -n+12 16.collinearity | grep -v '##' | sed -E 's/(.)-.+(Sc.+)\s(Sc.+)\t.+/\1\t\2\t\3/g' > blocks.tsv
# I PROCESSED THEM ON synteny.ipynb TO GET CHOORDINATES OF EACH GENE
```

```{r choords_blocks}
# IMPORT FILES
chr1 = read.delim("data/chr1.tsv", sep = "\t", header = TRUE)
chr2 = read.delim("data/chr2.tsv", sep = "\t", header = TRUE)

# I AIMED TO GET THE FIRST START CHOORD AND LAST END CHOORD FROM EACH BLOCK AND REMOVE DUPLICATED ROWS
chr1 = chr1 %>% arrange(block) %>% group_by(block) %>% mutate(start1=min(start), end1=max(end)) 
chr1 = chr1 %>% subset(select=c("sff", "block", "start1", "end1")) 
chr1 = chr1[!duplicated(chr1$block),]
chr1 = chr1 %>% subset(select=c("sff", "start1", "end1"))
write.table(chr1, "data/region1.tsv", quote=FALSE, sep='\t', row.names = FALSE)

chr2 = chr2 %>% arrange(block) %>% group_by(block) %>% mutate(start2=min(start), end2=max(end)) 
chr2 = chr2 %>% subset(select=c("sff", "block", "start2", "end2")) 
chr2 = chr2[!duplicated(chr2$block),]
chr2 = chr2 %>% subset(select=c("sff", "start2", "end2")) 
write.table(chr2, "data/region2.tsv", quote=FALSE, sep='\t', row.names = FALSE)

#blocks = cbind(chr1, chr2)
#blocks = blocks %>% subset(select=c("sff...1", "start1", "end1", "sff...5", "start2", "end2"))
#colnames(blocks) = c("sff1", "start1", "end1", "sff2", "start2", "end2")
```

```{bash script_bed_16sff, eval=FALSE, include=FALSE}
...
head data/sca_lengths.tab -n 16 | sed 's/Sc00000/S/' > data/sixt.sca.tab
cat data/region1.tsv | sed 's/Sc00000/S/' > data/link1.tsv
cat data/region2.tsv | sed 's/Sc00000/S/' > data/link2.tsv
rm data/region1.tsv data/region2.tsv 
```


```{r circos plot}
# GET CHOORDINATES
sff.bed = read.table("data/sixt.sca.tab")
sff.bed = data.frame(sff.bed$V1, 1, sff.bed$V2)
colnames(sff.bed) = c("Scaffold", "Start", "End")
write.table(sff.bed, "data/sff.bed", quote=FALSE, sep='\t', row.names = FALSE, col.names = FALSE)

exome = read.table("data/exome.bed")
exome = exome[,-2]

teRNAome = read.table("data/teRNAome.bed")
teRNAome = teRNAome[,-2]

teDNAome = read.table("data/teDNAome.bed")
teDNAome = teDNAome[,-2]

link1 = read.table("data/link1.tsv", header = TRUE)
link2 = read.table("data/link2.tsv", header = TRUE)

# PLOT THE CIRCOS

{png(file="figures/circular_genome.png", width=2000, height=2000)
circos.par("clock.wise"=TRUE, start.degree=86, gap.degree=c(rep(3,15), 8))
circos.genomicInitialize(sff.bed, plotType = c("axis","labels"), labels.cex = 4, axis.labels.cex=1)
circos.track(ylim = c(0, 0.05), bg.border = NA, bg.col = met.brewer("Cross", 16) , track.height = 0.05)
...
circos.genomicLink(region1 = link1, region2 = link2, col = met.brewer("Cross", 54))
circos.clear()
dev.off()
}
```

# CHARACTERIZATION OF TE-GENE RELATIONSHIP

```{bash upstream_tes, eval=FALSE, include=FALSE}
complementBed -i mrnas.bed -g sca_lengths.tab > intergenic_regions.bed
bedtools window -a mrnas.bed -b tes.bed -l 25000 -r 0 > upstream_tes.bed
#I SELECTED COLUMNS AND CLEANED THE TE NAMES
cut -f 1,2,4,12,13,20 upstream_tes.bed | sed -E 's/ID.+Name=(\w+.+):.+/\1/g' > upstream.tsv
```

```{r intergenic_lenghts}
intergenic = read.delim("data/intergenic_regions.bed", sep = "\t")
colnames(intergenic) = c("sff", "start", "end")
interg_lenghts=intergenic$end-intergenic$start
summary(interg_lenghts)
# MEAN = 17K
# I REMOVED MANY REGIONS SINCE THERE ARE MANY SHORT SCAFFOLDS
intnorm= which(interg_lenghts> 1000)
summary(intnorm)
sd(intnorm)
# MEAN = 23K +- 13K
ggplot() + geom_boxplot(aes("", intnorm))
# THEREBY, I DECIDED TO USE A 25KB WINDOW ABOVE
```

```{r}
upstream = read.delim("data/tesvsgenes.tsv", sep="\t")
upstream = upstream %>% mutate(
    len = end-start,
    tis = tss-start
         )
upstream <- upstream[upstream$tis >= 0, ]
max(upstream$tis)
# THERE ARE HIGH VALUES, I REMOVE THEM
upstream=upstream[upstream$tis <= 10000, ]

levels(factor(upstream$te))
upstream = upstream %>% mutate(
    niche = case_when(
      te %in% c("DNA", "DNA/CMC-EnSpm", "DNA/CMC-Transib", "DNA/hAT-Ac", "DNA/hAT-Tag1", "DNA/hAT-Tip100", "DNA/IS3EU", "DNA/Kolobok-Hydra", "DNA/Maverick", "DNA/Merlin", "DNA/MuLE-MuDR", "DNA/MULE-MuDR", "DNA/TcMar-Tc2", "Helitron", "DNA/PIF-Harbinger", "DNA/Novosib", "DNA/ot1", "DNA/Ginger") ~ "Class_II",
      T ~ "Class_I"
    )
  )

rects <- data.frame(ystart = seq(0.5,32.5,1), 
                    yend = seq(1.5,33.5,1), 
                    te=levels(factor(upstream$te)), 
                    niche=c(rep("Class_II",18), rep("Class_I", 15)))

# STATISTICAL TESTS, ARE THERE SIGNIFICANT DIFFERENCES?
tis.aov=aov(tis ~ te, data=upstream)
summary(tis.aov) #YES ***
len.aov=aov(len ~ te, data=upstream)
summary(len.aov) #YES ***
exp.aov=aov(mean_exp ~ te, data=upstream)
summary(exp.aov) #YES *
```

```{r}
#lenght
ggplot(upstream) + geom_boxplot(aes(x=log2(len), y=te, fill=niche))+ 
  geom_rect(data=rects, aes(xmin=0, xmax=16, ymin=ystart, ymax=yend, fill=niche), alpha=0.4)+
  scale_fill_manual(values = met.brewer("Stevens")) +
  guides(fill="none")+
  theme_hc()+
  labs(x="log2Lenght")+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
ggsave(file="figures/lenght.png", width=4, height=8, dpi=600)
```


```{r}
ggplot(upstream)+
  geom_density_ridges_gradient(aes(x=tis, y=te, fill=niche), color="white",alpha=0.2, quantile_lines = TRUE, quantiles=2) + 
  geom_rect(data=rects, aes(xmin=-2500, xmax=12500, ymin=ystart, ymax=yend, fill=niche), alpha=0.4)+
  scale_fill_manual(values = met.brewer("Stevens")) +
  guides(fill="none")+
  theme_hc()+
  labs(x="Distance from gene", y=NULL)+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
ggsave(file="figures/tis.png", width=4, height=8, dpi=600)
```

```{r}
ggplot(upstream) + geom_boxplot(aes(x=log2(mean_exp), y=te, fill=niche))+
  geom_rect(data=rects, aes(xmin=-15, xmax=15, ymin=ystart, ymax=yend, fill=niche), alpha=0.4)+
  scale_fill_manual(values = met.brewer("Stevens")) +
  guides(fill="none")+
  theme_hc()+
  labs(x="log2TPM")+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
ggsave(file="figures/exp.png", width=4, height=8, dpi=600)
```

```{r}
ggplot(upstream) + geom_bar(aes(x=after_stat(log2(count)), y=te, fill=niche)) + 
  geom_rect(data=rects, aes(xmin=0, xmax=15, ymin=ystart, ymax=yend, fill=niche), alpha=0.4)+
  scale_fill_manual(values = met.brewer("Stevens")) +
  guides(fill="none")+
  theme_hc()+
  labs(x="log2Count")+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
ggsave(file="figures/count.png", width=4, height=8, dpi=600)
```

```{r}
chars=upstream[,7:9]
correlations_chars=cor(chars)
chorddiag(correlations_chars)
```
