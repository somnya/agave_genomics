...
grep "tRNA" jcetz/AGAVEv3_TOT_snap1.all.gff | awk -F'\t' '$3 ~ /gene/ {print $0}' | awk '{$2="tRNA"; print $0}' | sed -E 's/ /\t/g' > trnas.gff # tRNAs
...
# REPEATOME RENAMING: Scaffold+R+ID
...
grep ">" raw/consensus_rep_families.fa | sed -E "s/.+#(.+)\s\(\s.+/\1/g" | sort | uniq -c
   ...
   1155 Unknown
      1 buffer
# FAMILY SIZE SEEMS NOT TO MATCH NUMBER OF REPEATS IN THE GENOME

cut -f 1-9 jcetz/AGAVEv3_TOT_snap1.all.gff | grep "repeat" |awk -F'\t' '$3 ~ /match/ {print $0}' | sed -E 's/ID=(\w+):\hit\:(\w+):.+\genus:(\w+).+/ID=\1R\2;Name=RM_\3:\2/g' | sed -E 's/ /\t/g' > repeats.gff
...
ggsave(file="sca1.png", width=12, height=8, dpi=300)
```
...
#{declare -a scaff=("Sc0000000" "Sc0000001" "Sc0000002" "Sc0000003" "Sc0000004" "Sc0000005" "Sc0000006" "Sc0000007" "Sc0000008" "Sc0000009" "Sc0000010" "Sc0000011" "Sc0000012" "Sc0000013" "Sc0000014" "Sc0000015")
rm exome.bed
rm teRNAome.bed
rm teDNAome.bed
#for x in  "${scaff[@]}"
...
```{bash}
grep -w "Sc0000000" ../gff/annotation.gff | grep -v "gene" | grep -v "mRNA" | grep -v "CDS" | grep -v "UTR" | cut -f 3,4 > first_scaffols_positions.tsv
```

```{r}
library(ggthemes)
first_scaffols_positions = read.delim("first_scaffols_positions.tsv", header = TRUE)
ggplot(first_scaffols_positions, aes(position, after_stat(count), fill = element)) +
  geom_density(position = "fill") + theme_void() 
```
